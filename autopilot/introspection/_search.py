...
def get_proxy_object_for_existing_process(**kwargs):
    ...
    **Examples:**
    ...
    object_path = kwargs.get('object_path', constants.AUTOPILOT_PATH)
    ...
    ROCKET_PILOT_DBUS_SERVICE_NAME = "name.glide.rocketpilot"
    found = False
    for _ in Timeout.default():
        if ROCKET_PILOT_DBUS_SERVICE_NAME in dbus_bus.list_names():
            found = True
            break

    if not found:
        raise RuntimeError("Cannot find target app DBUS service")

    return _make_proxy_object(
        _get_dbus_address_object(ROCKET_PILOT_DBUS_SERVICE_NAME, object_path, dbus_bus),
        emulator_base
    )
